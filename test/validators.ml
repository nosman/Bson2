...
let string_of_read_result r =
    Bson.Reader.sexp_of_read_result r
    |> Sexp.to_string
...
let validate_expected_read_results doc read_result_validators =
    let reader = Bson.Reader.create doc in
    List.iter
        read_result_validators
        ~f:(fun validator -> Bson.Reader.read_next reader |> validator)
...
let validate_exception_raised ~msg test_fun handler_fun =
    try
        test_fun ()
        assert_failure ~msg
    with exn ->
